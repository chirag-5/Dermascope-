...
const getConfig = () => ({
  ...
  confidenceThreshold: Number(process.env.ANTHROPIC_CONFIDENCE_THRESHOLD ?? 0.7),
  maxDetectionsPerType: Number(process.env.ANTHROPIC_MAX_DETECTIONS_PER_TYPE ?? 6),
  maxBoxAreaRatio: Number(process.env.ANTHROPIC_MAX_BOX_AREA_RATIO ?? 0.35),
});
...
const callAnthropicVision = async (base64, mimeType, prompt) => {
  const { apiKey, baseUrl, apiVersion, model } = getConfig();
...
export const validateAnthropicConfig = () => {
  ...
  if (!apiKey) {
    logError(
      '[Anthropic] ANTHROPIC_API_KEY is not set in backend/.env — annotation service disabled.'
    );
  ...
  serviceEnabled = true;
  logDebug('[Anthropic] Annotation service enabled.');
  logDebug(`[Anthropic] Model: ${model}`);
  logDebug(`[Anthropic] Confidence threshold: ${confidenceThreshold}`);
  return true;
};
...
export const analyzeImage = async (originalFilename) => {
  if (!serviceEnabled) {
    throw new Error(
      'Anthropic annotation service is disabled. Set ANTHROPIC_API_KEY in backend/.env.'
    );
  }
  ...
  const rawOutput = await callAnthropicVision(
    base64,
    mimeType,
    buildDetectionPrompt(image.width, image.height)
  );

  logDebug('[Anthropic] Raw model output:', rawOutput);

  const parsed = parseJsonContent(rawOutput);
  let { detections, rejected } = parseDetections(parsed, image.width, image.height);

  if (rejected.length > 0) {
    logDebug('[Anthropic] Rejected detections:', JSON.stringify(rejected, null, 2));
  }

  if (detections.length === 0) {
    logDebug('[Anthropic] No valid detections — using broad face-region fallback.');
    detections = buildFallbackBroadBoxes(image.width, image.height);
  }

  logDebug('[Anthropic] Final detections:', JSON.stringify(detections, null, 2));
